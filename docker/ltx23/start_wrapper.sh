...
if [ -n "$NETWORK_VOLUME" ] && [ -d "$NETWORK_VOLUME/models" ]; then
    ...
    for subdir in diffusion_models text_encoders vae loras mel_band_roformer; do
    ...
    mkdir -p "$MODEL_PATH/diffusion_models" "$MODEL_PATH/text_encoders" \
...
# Audio VAE (~365MB)
download_if_missing \
    "$MODEL_PATH/vae/LTX23_audio_vae_bf16.safetensors" \
...
# MelBandRoFormer (~456MB)
download_if_missing \
    "$MODEL_PATH/mel_band_roformer/MelBandRoformer_fp16.safetensors" \
